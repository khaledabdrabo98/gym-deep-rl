...
SIMPLE_KEYBOARD_ACTION = ['attack']


class Digdown(EnvSpec, ABC):
    def __init__(self, destination, name, world, resolution=(224, 224), max_episode_steps=6000, *args, **kwargs):
    ...
    def create_actionables(self):
        return [handlers.KeybasedCommandAction(k, v) for k, v in INVERSE_KEYMAP.items() if k in SIMPLE_KEYBOARD_ACTION]\
             + [handlers.EquipAction(['diamond_pickaxe', 'diamond_axe', 'diamond_shovel'], _other='diamond_pickaxe', _default='diamond_pickaxe')]
    ...
    def create_agent_start(self):
        return [handlers.SimpleInventoryAgentStart([dict(type='diamond_pickaxe', quantity='1'),
            ...
            dict(type='diamond_shovel', quantity='1')
        ])]
