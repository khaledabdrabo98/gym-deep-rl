...
SIMPLE_KEYBOARD_ACTION = ['forward', 'back', 'left', 'right', 'jump', 'sneak', 'sprint', 'use']


class Maze(EnvSpec, ABC):
    def __init__(self, destination, name, world, dense=False, resolution=(224, 224), max_episode_steps=600, *args, **kwargs):
    ...
    def create_rewardables(self):
        if self.dense:
            return [handlers.RewardForDistanceTraveledToCompassTarget(reward_per_block=1.0),
                ...
                handlers.RewardForTouchingBlockType([{'type':'redstone_block', 'behaviour':'constant', 'reward':'-0.5'}])
            ]
        else:
            return [handlers.RewardForDistanceTraveledToCompassTarget(reward_per_block=1.0),
                RewardForTimeTaken(-0.01)
    ...
    def create_agent_start(self):
        return [handlers.SimpleInventoryAgentStart([dict(type='dirt', quantity='64')])]
