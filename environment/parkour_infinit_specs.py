...
SIMPLE_KEYBOARD_ACTION = ['forward', 'back', 'left', 'right', 'jump', 'sneak', 'sprint']


class ParkourInfinit(EnvSpec, ABC):
    def __init__(self, destination, name, world, resolution=(224, 224), max_episode_steps=5000, *args, **kwargs):
    ...
    def create_observables(self):
        return [handlers.POVObservation(self.resolution, include_depth=True)]
    ...
    def create_rewardables(self):
        return [handlers.RewardForDistanceTraveledToCompassTarget(reward_per_block=1.0),
            ...
            handlers.RewardForTouchingBlockType([{'type':'redstone_block', 'behaviour':'constant', 'reward':'-10'}])
        ]
    ...
    def create_server_decorators(self):
        return [FixedNavigationDecorator(x=self.destination[0], y=self.destination[1], z=self.destination[2], block='diamond_block'),
            ParkourInfinitDrawBlockDecorator()
        ]
